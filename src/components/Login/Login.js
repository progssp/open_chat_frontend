...
export default function Login() {
  ...
  const [register_submit_btn_disabled, set_register_submit_btn_disabled] =
    useState(true);
    ...
    try {
      ...
      if (json_result.status) {
        ...
        window.localStorage.setItem(
          "cur_user",
          JSON.stringify(json_result.user),
        );
  ...
  async function handleRegister(evt) {
    ...
    if (returned_errors.length > 0) {
      showErrors(
        returned_errors,
        document.querySelector("#signup_error_alert"),
      );
      return;
    }
    ...
    try {
      ...
      if (json_result.status) {
        document.querySelector("#signup_success_alert").innerHTML =
          json_result.msg;
        document
          .querySelector("#signup_success_alert")
      ...
      document.querySelector("#signup_error_alert").innerHTML =
        "Request unavailable. Try again later.";
  ...
  async function checkUsernameAvailability(evt) {
    ...
    if (username.split(" ").join("").length !== 6) {
      ...
      document.getElementById("signup_error_alert").innerHTML =
        "username length should be 6 characters";
      setTimeout(() => {
    ...
    try {
      let response = await fetch(
        `${main_api_route}/user/check-username-availability`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
      ...
      if (result.status) {
        ...
        document.getElementById("signup_error_alert").classList.add("hidden");
        document
          .getElementById("signup_success_alert")
          .classList.remove("hidden");
        ...
        document.getElementById("signup_success_alert").classList.add("hidden");
        document
          .getElementById("signup_error_alert")
          .classList.remove("hidden");
      ...
      {login_visible ? (
        <div className="flex flex-col items-center overflow-y-scroll justify-center h-screen bg-slate-200 dark:bg-gray-700 px-2">
          <div className="flex flex-col items-center">
            ...
            />
            <h3 className="my-4 text-xl font-medium text-gray-900 dark:text-white">
              Open Chat - Sign in
            </h3>
          </div>
          <div className="rounded-md px-6 py-6 lg:px-8 shadow bg-slate-50 dark:bg-gray-500 w-full sm:w-3/4 md:w-[500px]">
            <form onSubmit={handleSubmit} className="space-y-6" action="#">
              <div
                id="success_alert"
                className="hidden w-auto p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
                role="alert"
              ></div>
              <div
                id="error_alert"
                className="hidden p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
                role="alert"
              ></div>
              ...
              <div className="flex">
                ...
                <button
                  className="ml-5 w-auto text-blue bg-slate-300 hover:text-white hover:bg-slate-600 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
        ...
        <div className="flex flex-col items-center justify-center overflow-y-scroll h-screen bg-slate-200 dark:bg-gray-700 px-2">
          ...
          <div className="rounded-md px-6 py-6 lg:px-8 shadow bg-slate-50 dark:bg-gray-500 w-full sm:w-3/4 md:w-[500px]">
            <form onSubmit={handleRegister} className="space-y-6" action="#">
              <div
                id="signup_success_alert"
                className="hidden w-auto p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
                role="alert"
              ></div>
              <div
                id="signup_error_alert"
                className="hidden p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
                role="alert"
              ></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <button
                    type="button"
                    className="underline w-auto text-white text-center"
              ...
              <div className="flex">
                <button
                  ...
                  id="signup_btn"
                  className="w-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                  disabled={register_submit_btn_disabled}
                >
                  Sign up
                </button>
                <img
