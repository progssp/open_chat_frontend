...
import { faPhone,faVolumeUp,faVolumeOff,faPlay,faStop } from "@fortawesome/free-solid-svg-icons";
import { subscribeSocket,subs_video_channel,backend_images_route} from '../../utilities/ExtraUtility';
...
export default function VideoCall(){
    ...
    function stopVideo(){
        ...
        localStream.getVideoTracks().forEach(track =>  track.enabled = (stop_video));
    ...
    async function createPeerConnection() {
        ...
        pc.onicecandidate = e => {
            ...
            setTimeout(()=>{
                video_channel.trigger('client-ice_candidate',{from:this_user,to:to,data:message});
            },1);
    ...
    function handleCallRequest(data){        
        if((parseInt(data.callee.callee_id) === parseInt(this_user))||(parseInt(data.from.id) === parseInt(this_user))){
            // console.log(data);
        ...
        if(parseInt(data.from.id) == parseInt(this_user)){
            ...
            timeout = setTimeout(() => {
                triggerChannelEvent(video_channel,'client-unanswered-call',{from:this_user,to:to});
                document.getElementById("call-request-modal").classList.add("invisible");
                stopBothVideoAndAudio();
                clearTimeout(timeout);
            },40000);
        ...
        if(parseInt(data.to) == parseInt(this_user)){
            // if(!pc){
                ...
                .catch(err => {
                    // console.error(err);
                    document.getElementById("call-request-modal").classList.add("invisible");
                    triggerChannelEvent(video_channel,'client-stop-call-',{from:this_user,to:to});
                });
            // }
        }
    ...
    async function makeCall(data) {
        if(parseInt(data.to) == parseInt(this_user)){
            ...
            video_channel.trigger('client-offer',{from:this_user,to:data.from,offer:offer});
    ...
    async function handleOffer(data) { 
        if(parseInt(data.to) == parseInt(this_user)){
            ...
            video_channel.trigger('client-answer',{from:this_user,to:to,answer:answer});
    ...
    function showVideoCallModal(){
        if(document.getElementById("video-call-modal").classList.contains("invisible")){
            document.getElementById("video-call-modal").classList.remove("invisible");
        }
    }
    ...
    return (
        <div>
            <div id="video-call-modal" className={`invisible fixed z-20 top-0 bg-[#000000a7] flex justify-center h-screen w-screen overflow-y-scroll`}>
                        ...
                        <div className="relative shadow h-screen bg-gray-800">
                            ...
                            <div className="relative px-2 rounded-md">                               
                                <div className="flex justify-center h-screen">
                                    <video muted id="rec_vid" className="object-cover sm:object-contain w-screen" autoPlay></video>
                                ...
                                <div className="absolute w-full left-0 bottom-0 flex flex-col items-start sm:flex-row sm:items-end justify-between p-3">
                                    <div className="flex flex-col sm:flex-row">
                                        <button onClick={endCall} className="m-[2px] rounded-full bg-red-600 w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 lg:w-13 lg:h-13 text-white font-bold text-sm" style={{rotate:"135deg"}}><FontAwesomeIcon icon={faPhone}/></button>
                                        <button onClick={stopVideo} className="m-[2px] rounded-full bg-blue-600 w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 lg:w-13 lg:h-13 text-white font-bold text-sm">{(stop_video)?<FontAwesomeIcon icon={faPlay}/>:<FontAwesomeIcon icon={faStop}/>}</button>
                                        <button onClick={stopAudio} className="m-[2px] rounded-full bg-blue-600 w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 lg:w-13 lg:h-13 text-white font-bold text-sm">{(stop_audio)?<FontAwesomeIcon icon={faVolumeOff}/>:<FontAwesomeIcon icon={faVolumeUp}/>}</button>
                                    </div>
                                    <div>
                                        <video muted id="sender_vid" className="bg-green-400 rounded-md object-contain w-6/12 sm:w-52 block md:block" autoPlay></video>
                                    </div>
                                </div>
                            </div>
            ...
            <div id="call-request-modal" className={`invisible fixed z-20 top-0 px-5 pb-12 bg-[#000000a7] flex items-center justify-center h-screen w-screen overflow-y-scroll sm:px-[15%] md:px-[25%]`}>
                    ...
                    <div className="relative w-auto">
                        ...
                        <div className="relative p-5 bg-white rounded-lg shadow dark:bg-gray-700">
                            <div className="flex flex-col justify-center items-center">
                                <span className="text-blue-800 dark:text-white text-md">Call from: </span>
                                <span id="call_from_span" className="hidden"></span>
                                <div id="call_from_image" style={{backgroundImage:`url('/system/storage/user_icons/WSDLYKsfAU8OHqp4tMaJ47eTrLX3GlSIMVnGAhZm.jpg')`}} className="w-28 h-28 rounded-full bg-no-repeat bg-center bg-cover"></div>
                                <span id="call_from_name_span" className="text-blue-800 dark:text-white text-xl"></span>
                                
                            </div>
                            <div className="p-4 flex justify-center">
                                <button id="accept_call_btn" onClick={acceptCall} className="rounded-full bg-green-600 w-12 h-12 text-white font-bold text-lg mx-2"><FontAwesomeIcon icon={faPhone}/></button>
                                <button id="reject_call_btn" onClick={rejectCall} style={{"rotate":"135deg"}} className="rounded-full bg-red-600 w-12 h-12 text-white font-bold text-lg mx-2"><FontAwesomeIcon icon={faPhone}/></button>
                            </div>
                        </div>
                    </div>
                </div>
